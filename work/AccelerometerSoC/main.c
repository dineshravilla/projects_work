...
int main(int argc, char *argv[]){
    ...
    while(bSuccess && (max_cnt == 0 || cnt < max_cnt)){
        if (ADXL345_IsDataReady(file)){
            ...
            if (bSuccess){
	            cnt++;
				// X is the X coord of the accelerometer
				int X = (int16_t)szXYZ[0]*mg_per_digi;
				// Iterate from -1024 to 1024 and break it into 8 chunks. Each chunk is a led.
				for(int i=0; i<8; i++){
					if((X < (-1024 + 256*i)) and (X > (-1024 + 256*(i-1)))){
						ledToLight = i;
					}
				}
                //printf("[%d]X=%d mg, Y=%d mg, Z=%d mg\r\n", cnt,(int16_t)szXYZ[0]*mg_per_digi, (int16_t)szXYZ[1]*mg_per_digi, (int16_t)szXYZ[2]*mg_per_digi);
				printf("Gsensor X value is %d. Lighting LEDs up to %d\n", cnt,(int16_t)szXYZ[0]*mg_per_digi, ledToLight);
                // show raw data, 
                //printf("X=%04x, Y=%04x, Z=%04x\r\n", (alt_u16)szXYZ[0], (alt_u16)szXYZ[1],(alt_u16)szXYZ[2]);
				LEDR_LightCount(ledToLight);
                usleep(100*1000);
				LEDR_AllOff();
            }
        }
